...
function initialiserIntervallesNotes() {
    ...
    getEtudiantByUE(selectedUEId).then(etudiants => {
        ...
        // Mapper en utilisant moyenne_ue du backend directement
        const tousEtudiants = etudiants.map(e => ({
            matricule:    e.matricule,
            prenom:       e.prenom,
            nom:          e.nom,
            moyenne:      e.moyenne_ue ?? 0,
            pointsJury:   (10 - (e.moyenne_ue ?? 0)).toFixed(4),
            est_repechable:       e.stats?.est_repechable ?? true,
            non_repechable_raison: e.stats?.non_repechable_raison ?? null,
            moyenne_calculable:   e.moyenne_calculable ?? false,
            ec:           e.ec
        }));

        // Éligibles = dans l'intervalle ET repêchables
        const eligibles = tousEtudiants.filter(e =>
            e.moyenne >= intervalle.min
            && e.moyenne < 10
            && e.moyenne_calculable
            && e.est_repechable
        );

        // Exclus de l'intervalle mais non repêchables (pour information)
        const exclus = tousEtudiants.filter(e =>
            e.moyenne >= intervalle.min
            && e.moyenne < 10
            && e.moyenne_calculable
            && !e.est_repechable
        ...
        if (eligibles.length === 0 && exclus.length === 0) {
        ...
        // Informer sur les exclus si nécessaire
        if (exclus.length > 0 && eligibles.length > 0) {
            Swal.fire({
                icon: 'warning',
                title: 'Étudiants non repêchables détectés',
                html: `
                    <p><strong>${exclus.length}</strong> étudiant(s) dans cet intervalle 
                    ne peuvent pas être repêchés :</p>
                    <ul class="text-start">
                        ${exclus.map(e => `
                            <li>
                                <strong>${e.prenom} ${e.nom}</strong> 
                                (${e.moyenne.toFixed(2)}/20)
                                — ${e.non_repechable_raison === 'non_compose_examen' 
                                    ? 'Non composé à l\'examen' 
                                    : 'Non repêchable'}
                            </li>
                        `).join('')}
                    </ul>
                    <p class="mt-2"><strong>${eligibles.length}</strong> étudiant(s) éligible(s) au repêchage.</p>
                `,
                confirmButtonText: 'Continuer avec les éligibles',
        ...
        } else if (exclus.length > 0 && eligibles.length === 0) {
            Swal.fire({
                icon: 'info',
                title: 'Aucun étudiant repêchable',
                html: `
                    <p>Les <strong>${exclus.length}</strong> étudiant(s) dans cet intervalle 
                    ne peuvent pas être repêchés :</p>
                    <ul class="text-start">
                        ${exclus.map(e => `
                            <li>
...
function loadUEs() {
    ...
    getMaquetteUEs(filters)
        .then(ues => {
            ...
            let nbUERepechees = 0;
            let nbUEsDelibierees = 0;
            if (ues && ues.length > 0) {
                ues.forEach(ue => {
                    const ueButton = document.createElement('button');
                    ueButton.type = 'button';
                    ueButton.className = 'btn btn-outline-primary ue-button w-100';
                    ueButton.innerHTML = `
                        <div class="text-start ">
                            <div class="fw-bold small d-flex justify-content-between">
                                ${ue.code} 
                                <div class="d-flex flex-column align-items-end bg-light p-1 rounded">
                                <span class="badge text-primary">${ue.estDelibiere ? '<i class="fas fa-check text-success"></i>' : ''}</span>
                                </div>
                            </div>
                    ...
                    ueButton.addEventListener('click', () => {
                        ...
                        verifierEvaluationsUE(selectedUEId).then(stats => {
                            ...
                            if (pourcentageComplets < 100) {
                                const raisons = stats.raisons_incompletude || {};
                                const hasPlusieursExamens = 'plusieurs_examens' in raisons;

                                // Construire les badges des raisons
                                const raisonsBadges = Object.entries(raisons).map(([raison, nb]) => {
                                    const isPlusieursExamens = raison === 'plusieurs_examens';
                                    const badgeClass = isPlusieursExamens ? 'bg-danger' : 'bg-warning text-dark';
                                    return `<span class="badge ${badgeClass} me-1 mb-1">${getReasonLabel(raison)} : ${nb}</span>`;
                                }).join('');

                                Swal.fire({
                                    title: hasPlusieursExamens
                                        ? `<span class="text-danger"><i class="fas fa-exclamation-triangle me-1"></i>Erreur de saisie détectée</span>`
                                        : `${pourcentageComplets == 0 ? '<p class="text-danger text-center mb-2">Veuillez saisir les notes.</p>' : 'Notes incomplètes'}`,
                                    html: `
                                        <div class="text-start">
                                            ${hasPlusieursExamens ? `
                                            <div class="alert alert-danger mb-3">
                                                <i class="fas fa-exclamation-triangle me-1"></i>
                                                <strong>Plusieurs notes d'examen ont été saisies pour un même EC.</strong>
                                                <br><small>Cette erreur bloque la délibération. Veuillez corriger la saisie.</small>
                                            </div>` : ''}
                                            ${pourcentageComplets == 0 ? '' : `
                                            <p class="fw-bold mb-2">Résumé des anomalies :</p>
                                            <div class="mb-3">
                                                <span class="badge bg-success me-2">Complets : ${etudiantsComplets}</span>
                                                <span class="badge bg-danger">Incomplets : ${etudiantsIncomplets}</span>
                                            </div>
                                            ${raisonsBadges ? `<div class="mb-3">${raisonsBadges}</div>` : ''}
                                            `}
                                            ${etudiantsIncomplets > 0 && pourcentageComplets != 0 ? `<p class="text-danger mb-2">Il y a ${etudiantsIncomplets} étudiant(s) avec des anomalies.</p>` : ''}
                                            ${etudiantsComplets > 0 && pourcentageComplets != 0 ? `<p class="text-success mb-2">Il y a ${etudiantsComplets} étudiant(s) complets.</p>` : ''}
                                            ${totalEtudiants > 0 && pourcentageComplets != 0 ? `<p class="mb-2">Sur un total de ${totalEtudiants} étudiant(s) inscrits à cette UE.</p>` : ''}
                                            ${pourcentageComplets > 0 ? `<p class="mb-2">Ce qui représente <strong>${pourcentageComplets.toFixed(2)}%</strong> des étudiants complets.</p>` : ''}
                                    ...
                                    `,
                                    icon: hasPlusieursExamens ? 'error' : 'warning',
                                    confirmButtonText: 'Voir la liste',
                                    confirmButtonColor: hasPlusieursExamens ? '#d33' : '#3085d6',
                                    showCancelButton: true,
                                            ...
                                            <tr>
                                                <td colspan="5" class="text-center py-4">
                                                    <div class="alert alert-info mb-0">
                                                        <i class="fas fa-info-circle me-1"></i>
                                                        Veuillez saisir les notes d'examen pour tous les étudiants afin de voir la liste complète.
                                                    </div>
                                                </td>
                                            </tr>
                                            `;
                                ...
                                // Vérification finale : pas d'erreur plusieurs_examens même si 100% complets
                                const raisons100 = stats.raisons_incompletude || {};
                                if ('plusieurs_examens' in raisons100) {
                                    Swal.fire({
                                        title: '<span class="text-danger">Erreur de saisie</span>',
                                        html: `<div class="alert alert-danger">
                                            <i class="fas fa-exclamation-triangle me-1"></i>
                                            <strong>Plusieurs notes d'examen ont été saisies pour un même EC.</strong>
                                            <br><small>Veuillez corriger la saisie avant de délibérer.</small>
                                        </div>`,
                                        icon: 'error',
                                        confirmButtonText: 'Voir la liste',
                                        ...
                                        if (r.isConfirmed) afficherListeEtudiantsIncomplets(stats.liste_etudiants_incomplets);
                                    });
                                } else {
...
function getDeliberationDeUE(ueId) {
    ...
    fetch(`deliberationUeController.php?action=getDeliberationDeUE&idUE=${encodeURIComponent(ueId)}`)
        ...
        .then(data => {
            ...
            // Générer HTML pour tous les validés (y compris repêchés)
            let totalValidesHtml = '';
            [...valides, ...repeches].forEach(e => {
                const moyenne = e.moyenne_ue !== null && e.moyenne_ue !== undefined ? e.moyenne_ue.toFixed(2) : 'N/A';
                ...
                totalValidesHtml += `
                    <tr class="table-success">
                        ...
                        <td>${escapeHtml(e.nom || '')}</td>
                        <td><span class="badge bg-success">Validé</span></td>
                        <td class="text-center">${moyenne < 10 ? 10 : moyenne}</td>
                    </tr>
                `;
            });

            if (totalValidesHtml === '') {
                totalValidesHtml = '<tr><td colspan="6" class="text-center">Aucun étudiant validé</td></tr>';
            }
                ...
                <div class="card border-info">
                    <div class="card-body">
                            ...
                            <div class="col-md-2 col-6 mb-2">
                             ...
                             <div class="col-md-2 col-6 mb-2">
                                <div class="border rounded p-3 text-center">
                                    ...
                                    <h3 class="text-danger mb-0">${ue.barre == undefined || ue.barre == null || ue.barre == 10 ? 'Pas de repêchage' : parseFloat(ue.barre).toFixed(2)}</h3>
                                </div>
                            </div>
                        </div>
                        ...
                        <ul class="nav nav-tabs" id="deliberationTabs" role="tablist">
                            ...
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="total-valides-tab" data-bs-toggle="tab" data-bs-target="#total-valides" type="button" role="tab">
                                    Total validés <span class="badge bg-info ms-1">${nbValides + nbRepeches}</span>
                                </button>
                            </li>
                        </ul>
                        ...
                        <div class="tab-content p-3 border border-top-0 rounded-bottom" id="deliberationTabContent">
                            ...
                            <div class="tab-pane fade" id="total-valides" role="tabpanel">
                                <div class="table-responsive">
                                    <table class="table table-sm table-hover">
                                        <thead>
                                            <tr>
...
function afficherListeEtudiantsIncomplets(etudiantsIncomplets) {
    ...
    etudiantsIncomplets.forEach(etudiant => {
        ...
        // Détail par anomalie avec badge coloré selon le type
        const detailBadges = anomalies.map(a => {
            const label = getReasonLabel(a.raison || a.type || '');
            const isPlusieursExamens = (a.raison || a.type) === 'plusieurs_examens';
            const badgeClass = isPlusieursExamens ? 'bg-danger' : 'bg-warning text-dark';
            const ecNom = a.ec_nom ? `<strong>${a.ec_nom}</strong> : ` : '';
            return `<span class="badge ${badgeClass} me-1 mb-1">${ecNom}${label}</span>`;
        }).join('');

        // Indicateur spécial si plusieurs examens détectés
        const hasPlusieursExamens = anomalies.some(a => (a.raison || a.type) === 'plusieurs_examens');
        const alertRow = hasPlusieursExamens
            ? `<span class="badge bg-danger ms-1"><i class="fas fa-exclamation-triangle"></i> Erreur saisie</span>`
            : '';

        html += `
            <tr ${hasPlusieursExamens ? 'class="table-danger"' : ''}>
                <td><small>${etudiant.matricule}</small></td>
                <td>${etudiant.nom} ${alertRow}</td>
                <td class="text-center">
                    <span class="badge bg-danger">${etudiant.ec_manquants || 0}</span>
                </td>
                <td>
                    <div class="d-flex flex-wrap gap-1">
                        ${detailBadges || '<small class="text-muted">—</small>'}
                    </div>
                </td>
            </tr>
        `;
...
document.getElementById('btnRunSimu').addEventListener('click', () => {
    if (!selectedUEId || !seuilInput.value) {
    ...
    lancerSimulation(seuilInput.value, selectedUEId, currentIntervalle);
});
    ...
    Swal.fire({
        ...
        html: `
            <div class="text-start">
                ...
                <p><strong>Seuil :</strong> À partir de ${seuilInput.value}/20</p>
...
let ecDataTable = null;

function loadECs(ueId, noteEtudiantsParEC = null) {

    const ecTableContainer = document.getElementById('ecTableContainer');
    if (ecTableContainer && ecTableContainer.classList.contains('d-none')) {
    ...
    promesseEtudiants
        .then(etudiants => {
            ...
            if (!etudiants || etudiants.length === 0) {
                ...
                actualiserStatsUE({ min: 0, max: 0, moyenne: 0, effectif: 0, reussite: 0, echec: 0 });
                return;
            }

            // Précalcul des moyennes et statuts une seule fois
            const etudiantsTraites = etudiants.map(etudiant => {
                const moyenne        = etudiant.moyenne_ue ?? 0;
                const reussi         = moyenne >= 10;
                const estRepechable  = etudiant.stats?.est_repechable ?? false;
                const raison         = etudiant.stats?.non_repechable_raison ?? null;
                const moyenneCalc    = etudiant.moyenne_calculable ?? false;

                return { ...etudiant, moyenne, reussi, estRepechable, raison, moyenneCalc };
            });

            // Mise à jour des intervalles en O(n)
            intervalleNote.forEach(intervalle => {
                intervalle.nbEtudiants = 0;
            });
            etudiantsTraites.forEach(etudiant => {
                intervalleNote.forEach(intervalle => {
                    if (etudiant.moyenne >= intervalle.min && etudiant.moyenne < intervalle.max) {
            ...
            intervalleNote.forEach(intervalle => {
                const badge = document.getElementById(`badgeIntervalle${intervalle.min}`);
                if (badge) {
                    badge.textContent = intervalle.nbEtudiants;
                    badge.title = `${intervalle.nbEtudiants} étudiant(s) entre ${intervalle.min} et ${intervalle.max}`;
                }
            });
            ...
            etudiantsTraites.forEach(etudiant => {
                const { moyenne, reussi, estRepechable, raison, moyenneCalc } = etudiant;

                effectif++;
                moyenneGenerale += moyenne;
                reussi ? nbReussite++ : nbEchec++;
                ...
                let badgeStatut;
                if (!moyenneCalc) {
                    badgeStatut = `<span class="badge bg-secondary">Non calculable</span>`;
                } else if (reussi) {
                    badgeStatut = `<span class="badge bg-success">validé</span>`;
                } else {
                    badgeStatut = `<span class="badge bg-danger">Non validé</span>`;
                }

                // Badge repêchage
                let badgeRepechage = '';
                if (!reussi && moyenneCalc) {
                    if (estRepechable) {
                        badgeRepechage = `
                            <span class="badge bg-warning text-dark ms-1" 
                                  title="Éligible au repêchage">
                                <i class="fas fa-life-ring me-1"></i>Repêchable
                            </span>`;
                    } else {
                        const tooltip = raison ? `Non repêchable : ${raison}` : 'Non repêchable';
                        badgeRepechage = `
                            <span class="badge bg-secondary ms-1" title="${tooltip}">
                                <i class="fas fa-ban me-1"></i>Non repêchable
                            </span>`;
                    }
                }
                ...
                row.innerHTML = `
                    ...
                    <td data-order="${moyenne.toFixed(4)}">
                        <span class="badge ${reussi ? 'bg-success' : moyenne > 0 ? 'bg-danger' : 'bg-secondary'} p-2">
                            ${moyenneCalc ? moyenne.toFixed(2) : 'N/A'}
                        </span>
                    </td>
            ...
            ecTableBody.appendChild(fragment);

            // Initialisation DataTables
            ecDataTable = $('#ecTable').DataTable({
                destroy: true,
                pageLength: 10,
                ...
                buttons: [
                    {
                        ...
                        title: `Export UE - ${new Date().toLocaleDateString('fr-FR')}`,
            ...
            const stats = {
                min: minMoyenne === Infinity ? 0 : minMoyenne,
                max: maxMoyenne,
            ...
            getStatUE(ueId).then(serverStats => {
                if (serverStats && serverStats.length > 0) {
                    const s = serverStats[0];
                    actualiserStatsUE({
                        ...stats,
                        present: parseInt(s.nombreComposes || 0),
                        absent: parseInt(s.nombreNonComposes || 0)
                    });
                }
            });
        })
        .catch(error => {
            console.error('Erreur loadECs:', error);
            const ecTableBody = document.getElementById('ecTableBody');
            if (ecTableBody) {
                ecTableBody.innerHTML = `
                    <tr>
            ...
            actualiserStatsUE({ min: 0, max: 0, moyenne: 0, effectif: 0, reussite: 0, echec: 0, present: 0, absent: 0 });
        });
}
// ============================================================
// Helper : calcul de la moyenne d'un étudiant
// (évite la duplication dans la fonction principale)
// ============================================================
...
const btnDeliberer = document.getElementById('btnDeliberer');
if (btnDeliberer) {
    btnDeliberer.addEventListener('click', () => {
        // Logique pour lancer la délibération
        lancerDeliberation()
    });
}else {
    console.warn('Bouton de délibération non trouvé dans le DOM');
}
function lancerDeliberation() {
    if (!selectedUEId) {
        Swal.fire('Information', 'Veuillez sélectionner une UE pour délibérer', 'info');
        return;
    }

    Swal.fire({
        title: 'Confirmer la délibération',
        text: 'Êtes-vous sûr de vouloir délibérer cette UE ?',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        ...
        cancelButtonText: 'Annuler',
        allowOutsideClick: () => !Swal.isLoading() // Empêche de fermer pendant le chargement
    }).then((result) => {
        if (result.isConfirmed) {
            ...
            // --- Étape 2 : Lancer le Fetch ---
            fetch(`deliberationUeController.php`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({action: 'lancerDeliberationUE', idUE: selectedUEId})
            })
            .then(response => response.json())
            .then(data => {
                // Le prochain Swal.fire remplacera automatiquement le spinner
                if (data.success) {
                    Swal.fire('Succès', 'La délibération a été effectuée avec succès', 'success');
                    loadUEs();
                } else {
                    Swal.fire('Erreur', data.message || 'Erreur lors de la délibération', 'error');
                }
            })
            .catch(() => {
                Swal.fire('Erreur', 'Erreur réseau lors de la délibération', 'error');
            });
        }
    });
}